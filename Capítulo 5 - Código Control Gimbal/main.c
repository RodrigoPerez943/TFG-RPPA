...
int main(int argc, char **argv)
{
    ...
    // Atención: cuando el programa se cuelga, Ctrl-C generará el archivo de volcado de memoria (coredump)
    signal(SIGTERM, DjiUser_NormalExitHandler);

    printf("Iniciando la preparación del sistema\n");
    /*!< Paso 1: Preparar el entorno del sistema, como osal, hal uart, función de consola, etc. */
    returnCode = DjiUser_PrepareSystemEnvironment();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al preparar el entorno del sistema");
        printf("Error al iniciar la preparación del sistema\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }

    printf("Iniciando el relleno de datos de la aplicación\n");
    /*!< Paso 2: Completa la información de tu aplicación en dji_sdk_app_info.h y usa esta interfaz para completarla. */
    returnCode = DjiUser_FillInUserInfo(&userInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al completar la información del usuario, por favor verifica la configuración");
        printf("Error al iniciar el relleno de datos de la aplicación\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }

    printf("Iniciando el sistema PSDK\n");
    /*!< Paso 3: Inicializar el núcleo del Payload SDK con la información de tu aplicación. */
    returnCode = DjiCore_Init(&userInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error en la inicialización del núcleo");
        printf("Error al iniciar el sistema PSDK\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }

    printf("Obteniendo información básica de la aeronave\n");
    returnCode = DjiAircraftInfo_GetBaseInfo(&aircraftInfoBaseInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al obtener la información básica de la aeronave");
        printf("Error al obtener la información básica de la aeronave\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    printf("Información básica de la aeronave obtenida con éxito\n");

    if (aircraftInfoBaseInfo.mountPositionType != DJI_MOUNT_POSITION_TYPE_PAYLOAD_PORT) {
        printf("Obteniendo versión de la aeronave\n");
        returnCode = DjiAircraftInfo_GetAircraftVersion(&aircraftInfoVersion);
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al obtener la versión de la aeronave");
            printf("Error al obtener la versión de la aeronave\n");
        } else {
            USER_LOG_INFO("La versión de la aeronave es V%02d.%02d.%02d.%02d", aircraftInfoVersion.majorVersion,
                          aircraftInfoVersion.minorVersion, aircraftInfoVersion.modifyVersion,
                          aircraftInfoVersion.debugVersion);
            printf("Versión de la aeronave obtenida con éxito\n");
        }
    }

    printf("Estableciendo alias de la aplicación\n");
    returnCode = DjiCore_SetAlias("PSDK_APPALIAS");
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al establecer el alias");
        printf("Error al establecer el alias de la aplicación\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    printf("Alias de la aplicación establecido con éxito\n");

    printf("Estableciendo versión de firmware\n");
    returnCode = DjiCore_SetFirmwareVersion(firmwareVersion);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al establecer la versión de firmware");
        printf("Error al establecer la versión de firmware\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    printf("Versión de firmware establecida con éxito\n");

    printf("Estableciendo número de serie\n");
    returnCode = DjiCore_SetSerialNumber("PSDK12345678XX");
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al establecer el número de serie");
        printf("Error al establecer el número de serie\n");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    printf("Número de serie establecido con éxito\n");

    /*!< Paso 4: Inicializar los módulos seleccionados mediante macros en dji_sdk_config.h . */
#ifdef CONFIG_MODULE_SAMPLE_POWER_MANAGEMENT_ON
    printf("Iniciando módulo de gestión de energía\n");
    ...
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al registrar el manejador de alta potencia");
        printf("Error al registrar el manejador de alta potencia\n");
    } else {
        printf("Manejador de alta potencia registrado con éxito\n");
    }

    returnCode = DjiTest_PowerManagementStartService();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al iniciar el servicio de gestión de energía");
        printf("Error al iniciar el servicio de gestión de energía\n");
    } else {
        printf("Servicio de gestión de energía iniciado con éxito\n");
    }
#endif

#ifdef CONFIG_MODULE_SAMPLE_DATA_TRANSMISSION_ON
    printf("Iniciando módulo de transmisión de datos\n");
    returnCode = DjiTest_DataTransmissionStartService();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al iniciar el módulo de transmisión de datos");
        printf("Error al iniciar el módulo de transmisión de datos\n");
    } else {
        printf("Módulo de transmisión de datos iniciado con éxito\n");
    }
#endif
    ...
    if (aircraftInfoBaseInfo.mountPosition == DJI_MOUNT_POSITION_EXTENSION_PORT &&
        ...
        printf("Iniciando servicios específicos para M300/M350 en puerto de extensión\n");

        printf("Iniciando servicio de interacción de widgets\n");
        returnCode = DjiTest_WidgetInteractionStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de interacción de widgets");
            printf("Error al iniciar el servicio de interacción de widgets\n");
        } else {
            printf("Servicio de interacción de widgets iniciado con éxito\n");
        }

        printf("Iniciando servicio de altavoz de widget\n");
        returnCode = DjiTest_WidgetSpeakerStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de altavoz de widget");
            printf("Error al iniciar el servicio de altavoz de widget\n");
        } else {
            printf("Servicio de altavoz de widget iniciado con éxito\n");
        }

#ifdef CONFIG_MODULE_SAMPLE_UPGRADE_ON
        printf("Iniciando servicio de actualización\n");
        ...
        if (DjiTest_UpgradeStartService(&linuxUpgradePlatformOpt, testUpgradeConfig) !=
            ...
            USER_LOG_ERROR("Error al iniciar el servicio de actualización");
            printf("Error al iniciar el servicio de actualización\n");
        } else {
            printf("Servicio de actualización iniciado con éxito\n");
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_MOP_CHANNEL_ON
        printf("Iniciando servicio de canal MOP\n");
        returnCode = DjiTest_MopChannelStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de canal MOP");
            printf("Error al iniciar el servicio de canal MOP\n");
        } else {
            printf("Servicio de canal MOP iniciado con éxito\n");
        }
#endif
    } else {
#ifdef CONFIG_MODULE_SAMPLE_CAMERA_EMU_ON
        printf("Iniciando servicio de emulación de cámara base\n");
        returnCode = DjiTest_CameraEmuBaseStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de emulación de cámara base");
            printf("Error al iniciar el servicio de emulación de cámara base\n");
        } else {
            printf("Servicio de emulación de cámara base iniciado con éxito\n");
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_CAMERA_MEDIA_ON
        printf("Iniciando servicio de emulación de cámara media\n");
        returnCode = DjiTest_CameraEmuMediaStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de emulación de cámara media");
            printf("Error al iniciar el servicio de emulación de cámara media\n");
        } else {
            printf("Servicio de emulación de cámara media iniciado con éxito\n");
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_FC_SUBSCRIPTION_ON
        printf("Iniciando servicio de suscripción FC\n");
        returnCode = DjiTest_FcSubscriptionStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de suscripción FC");
            printf("Error al iniciar el servicio de suscripción FC\n");
        } else {
            printf("Servicio de suscripción FC iniciado con éxito\n");
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_GIMBAL_EMU_ON
        printf("Iniciando servicio de emulación de gimbal\n");
        if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_SKYPORT_V2 ||
            ...
            if (DjiTest_GimbalStartService() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("Error al iniciar el servicio de emulación de gimbal");
                printf("Error al iniciar el servicio de emulación de gimbal\n");
            } else {
                printf("Servicio de emulación de gimbal iniciado con éxito\n");
            }
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_XPORT_ON
        printf("Iniciando servicio XPort\n");
        if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_XPORT) {
            if (DjiTest_XPortStartService() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("Error al iniciar el servicio XPort");
                printf("Error al iniciar el servicio XPort\n");
            } else {
                printf("Servicio XPort iniciado con éxito\n");
            }
        }
#endif

#ifdef CONFIG_MODULE_SAMPLE_WIDGET_ON
        printf("Iniciando servicio de widget\n");
#if DJI_USE_WIDGET_INTERACTION
        printf("Iniciando servicio de interacción de widget\n");
        returnCode = DjiTest_WidgetInteractionStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de interacción de widget");
            printf("Error al iniciar el servicio de interacción de widget\n");
        } else {
            printf("Servicio de interacción de widget iniciado con éxito\n");
        }
#else
        returnCode = DjiTest_WidgetStartService();
        if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            USER_LOG_ERROR("Error al iniciar el servicio de widget");
            printf("Error al iniciar el servicio de widget\n");
        } else {
            printf("Servicio de widget iniciado con éxito\n");
        }
#endif
#endif
        ...
        printf("Iniciando servicio de colaboración de carga útil\n");
        if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_SKYPORT_V2 ||
            aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_XPORT) {
            returnCode = DjiTest_PayloadCollaborationStartService();
            if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("Error al iniciar el servicio de colaboración de carga útil");
                printf("Error al iniciar el servicio de colaboración de carga útil\n");
            } else {
                printf("Servicio de colaboración de carga útil iniciado con éxito\n");
            }
        }
#endif
    ...
    printf("Iniciando servicio de personalización HMS\n");
    returnCode = DjiTest_HmsCustomizationStartService();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al iniciar el servicio de personalización HMS");
        printf("Error al iniciar el servicio de personalización HMS\n");
    } else {
        printf("Servicio de personalización HMS iniciado con éxito\n");
    }
#endif

    /*!< Paso 5: Indica al DJI Pilot que estás listo. */
    printf("Indicando al DJI Pilot que la aplicación está lista\n");
    returnCode = DjiCore_ApplicationStart();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Error al iniciar la aplicación del SDK");
        printf("Error al iniciar la aplicación del SDK\n");
    } else {
        printf("Aplicación del SDK iniciada con éxito\n");
    }

    printf("Creando hilo de monitorización\n");
    if (pthread_create(&s_monitorThread, NULL, DjiUser_MonitorTask, NULL) != 0) {
        USER_LOG_ERROR("Error al crear la tarea de monitorización");
        printf("Error al crear el hilo de monitorización\n");
    } else {
        printf("Hilo de monitorización creado con éxito\n");
    }

    printf("Estableciendo nombre al hilo de monitorización\n");
    if (pthread_setname_np(s_monitorThread, "monitor task") != 0) {
        USER_LOG_ERROR("Error al establecer el nombre de la tarea de monitorización");
        printf("Error al establecer el nombre del hilo de monitorización\n");
    } else {
        printf("Nombre del hilo de monitorización establecido con éxito\n");
    }
